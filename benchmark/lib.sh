...
#
# The entry is generated by jq (so quotes/backslashes/newlines in any field
# are escaped correctly), and the file is rewritten atomically: jq reads the
# existing array, appends the entry into a temp file in the same directory,
# the result is validated, then mv'd over results.json. After any sequence of
# calls — including ones starting from a corrupt file — results.json is a
# valid JSON array.
save_result() {
    local category="$1"
    local name="$2"
    ...
    entry=$(jq -n \
        ...
        '{
            ...
            category: $category,
            name: $name,
            time: ($time | tonumber? // 0),
            output: $output,
            exit_code: ($exit_code | tonumber? // 1)
        }')

    # Append atomically: rewrite into a same-directory temp file and mv, so a
    # crash mid-write can never leave a truncated results.json behind.
    local tempfile
    tempfile=$(mktemp "$RESULTS_DIR/.results.json.XXXXXX")
    if [ -f "$RESULTS_FILE" ] \
        && jq -e 'type == "array"' "$RESULTS_FILE" >/dev/null 2>&1; then
        jq --argjson entry "$entry" '. + [$entry]' "$RESULTS_FILE" > "$tempfile"
    else
        # Missing, empty, or corrupt file: start a fresh array.
        jq -n --argjson entry "$entry" '[$entry]' > "$tempfile"
    fi
    if ! jq empty "$tempfile" 2>/dev/null; then
        rm -f "$tempfile"
        echo "save_result: generated invalid JSON, refusing to write $RESULTS_FILE" >&2
        return 1
    fi
