...
| B1. gc_root 守卫宏 — GC_ROOTS_SCOPE | ✅（#136 删除：GC 不再需要手写 root；#150 收集点已迁到分配时 + gc_gate 门） |
| B4. GC 分配时收集 + gc_gate 门 + arena 固定预留 | ✅（#136 删 gc_roots/proc_grow/fixup；#150 从 opcode 边界迁到分配时 + 门） |
| B2. 统一错误处理 — 返回约定规范化 | ✅ |
