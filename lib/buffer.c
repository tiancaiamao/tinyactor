 ...
 *   raw_push_str(h, s)       -> Int new length, or -1 (closed / non-string)
 *   raw_push_byte(h, n)      -> Int new length, or -1 (closed / n outside
 *                               0..255 — defense in depth; lib/buffer.ta
 ...
 *                               (closed; bounds are pre-validated in TA)
 *   raw_close(h)             -> Int 1, idempotent (double close is also
 *                               1); -1 only for a handle out of table range
 *   raw_length(h)            -> Int length, or -1 (closed)
 *
...
/* Validate a live handle and return its slot, or NULL if invalid
 * (out of range, or a freed slot marked by len == -1). */
...
/* Release the handle's storage. Idempotent: closing an already-closed
 * in-range handle also returns 1 (docs/c-module.md §5). */
