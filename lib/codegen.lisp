...
;; Defined as zero-arg functions because (define NAME VAL) is
;; not supported at top level by the Lisp host.
;; ============================================================

(define (OP_PUSH_NIL) 0)
(define (OP_PUSH_TRUE) 1)
(define (OP_PUSH_FALSE) 2)
(define (OP_PUSH_INT8) 3)
(define (OP_PUSH_INT) 4)
(define (OP_PUSH_SYM) 5)
(define (OP_PUSH_STRING) 6)
(define (OP_LOAD) 7)
(define (OP_STORE) 8)
(define (OP_CONS) 9)
(define (OP_CAR) 10)
...
(define (OP_LE) 19)
(define (OP_IS_NIL) 20)
(define (OP_IS_PAIR) 21)
(define (OP_IS_INT) 22)
(define (OP_IS_STRING) 23)
(define (OP_IS_BYTES) 24)
(define (OP_IS_PID) 25)
(define (OP_JUMP) 26)
(define (OP_JUMP_IF_FALSE) 27)
(define (OP_POP) 28)
(define (OP_DUP) 29)
(define (OP_CLOSURE) 30)
(define (OP_CALL) 31)
(define (OP_TAIL_CALL) 32)
(define (OP_RET) 33)
(define (OP_SPAWN) 34)
(define (OP_SPAWN_MAIN) 35)
(define (OP_SPAWN_CLOS) 36)
(define (OP_SEND) 37)
(define (OP_RECV) 38)
(define (OP_RECV_PEEK) 39)
(define (OP_RECV_COMMIT) 40)
(define (OP_SELF) 41)
(define (OP_MONITOR) 42)
(define (OP_PRINT) 43)
(define (OP_HALT) 44)
(define (OP_MATCH_INT) 45)
(define (OP_MATCH_SYM) 46)
(define (OP_MATCH_NIL) 47)
(define (OP_MATCH_PAIR) 48)
(define (OP_MATCH_JUMP) 49)
(define (OP_STR_LEN) 50)
(define (OP_STR_CONCAT) 51)
(define (OP_STR_SLICE) 52)
(define (OP_STR_EQ) 53)
(define (OP_CCALL) 54)
(define (OP_ENTER) 55)
...
(define (inline_op sym)
  ...
  (if (= sym 'car) (OP_CAR)
  (if (= sym 'cdr) (OP_CDR)
  (if (= sym 'null?) (OP_IS_NIL)
  (if (= sym 'pair?) (OP_IS_PAIR)
  (if (= sym 'int?) (OP_IS_INT)
  (if (= sym 'string?) (OP_IS_STRING)
  (if (= sym 'bytes?) (OP_IS_BYTES)
  (if (= sym 'pid?) (OP_IS_PID)
  (if (= sym 'print) (OP_PRINT)
  (if (= sym 'string-length) (OP_STR_LEN)
  (if (= sym 'string-concat) (OP_STR_CONCAT)
  (if (= sym 'string-slice) (OP_STR_SLICE)
  (if (= sym 'string-eq) (OP_STR_EQ) -1)))))))))))))))))))))))
...
(define (compile_expr b expr tail state)
  (if (int? expr)
      (begin
        (if (>= expr 0)
            (if (< expr 128)
                (begin (buf.push_byte b (OP_PUSH_INT8)) (buf.push_byte b expr))
                (begin (buf.push_byte b (OP_PUSH_INT)) (emit_i64 b expr)))
            (if (>= expr -128)
                (begin (buf.push_byte b (OP_PUSH_INT8)) (buf.push_byte b (+ expr 256)))
                (begin (buf.push_byte b (OP_PUSH_INT)) (emit_i64 b expr))))
        state)
      (if (string? expr)
          (begin
            ...
            state)
          (if (null? expr)
              (begin (buf.push_byte b (OP_PUSH_NIL)) state)
                                                        (if (pair? expr)
                      ...
                      (if (null? slot)
                          (let fn_names (st_fn_names state)
                            (let fid (find_fn fn_names expr)
                                                                                          (if (null? fid)
                                  (if (= expr (mk_true))
                                      (begin (buf.push_byte b (OP_PUSH_TRUE)) state)
                                      (if (= expr (mk_false))
                                          (begin (buf.push_byte b (OP_PUSH_FALSE)) state)
                                          (begin (buf.push_byte b (OP_PUSH_NIL)) state)))
                                  (begin (buf.push_byte b (OP_CLOSURE))
                                         (emit_u32 b fid)
                                         (emit_u32 b 0)
                                         state))))
                          (begin (buf.push_byte b (OP_LOAD))
                                                                  (emit_u32 b slot)
                                                           state)))))))))
    ...
    (if (null? quoted)
        ...
        (if (int? quoted)
            (begin
              (if (>= quoted 0)
                  (if (< quoted 128)
                      (begin (buf.push_byte b (OP_PUSH_INT8)) (buf.push_byte b quoted))
                      (begin (buf.push_byte b (OP_PUSH_INT)) (emit_i64 b quoted)))
                  (if (>= quoted -128)
                      (begin (buf.push_byte b (OP_PUSH_INT8)) (buf.push_byte b (+ quoted 256)))
                      (begin (buf.push_byte b (OP_PUSH_INT)) (emit_i64 b quoted))))
              state)
            (if (pair? quoted)
                (begin (buf.push_byte b (OP_PUSH_NIL)) state)
...
(define (compile_special b head args tail state)
  (if (= head 'spawn)
      ...
      (if (= head 'send)
          ...
          (if (= head 'recv)
              (begin (buf.push_byte b (OP_RECV)) state)
              (if (= head 'self)
                  (begin (buf.push_byte b (OP_SELF)) state)
                  (if (= head 'monitor)
                      (let s1 (compile_expr b (car args) 0 state)
...
(define (compile_general_call b head args tail state)
  (let s1 (compile_expr b head 0 state)
    (let s2 (compile_args b args s1)
      (buf.push_byte b (if (= tail 1) (OP_TAIL_CALL) (OP_CALL)))
                                        ...
                                        (let final_total_fns (cdr (cdr tl_result))
                                          (if (= has_top 0)
                                              (let main_fid (find_main_fn fn_names)
                                                (if (>= main_fid 0)
                                                    (begin (buf.push_byte b (OP_SPAWN_MAIN)) (emit_u32 b main_fid))
                                                    'nil))
                                              'nil)
                                          (buf.push_byte b (OP_PUSH_NIL))
